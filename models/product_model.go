...
func NewProductModel(db *sql.DB, cache *redis.Client) *ProductModel {
	return &ProductModel{db, cache}
}
...
func (m *ProductModel) GetByID(id int64) (*Product, error) {
	...
	err = m.cache.Set(cacheKey, string(productJSON), cacheExpiration*time.Minute).Err()
	if err != nil {
